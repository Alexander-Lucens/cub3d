...
{
	if (game->map.exit.position.x == -1 && \
		game->map.exit.position.y == -1 && x == 'E')
	{
		game->map.matrix[pos.y][pos.x] = x;
		game->map.exit.position = pos;
		return ;
	}
	if (game->map.player.position.x == -1 && \
		game->map.player.position.y == -1 && x == 'P')
	{
		game->map.matrix[pos.y][pos.x] = x;
...
static void	filler(t_game *game, int i, int j, int x)
{
	...
	else if (game->map.map[x] == 'Q')
	{
		game->map.matrix[i][j] = 'Q';
		game->map.count_enemies++;
	}
...
void	map_str_to_matrix(t_game *game)
{
	...
	game->map.count_enemies = 0;
	i = 0;
	game->map.player = init_tgeo(init_tpos(-1, -1), init_tpos(1, 0));
	game->map.exit = init_tgeo(init_tpos(-1, -1), init_tpos(0, 0));
	...
	fill_enemies_position(game);
	if (game->map.player.position.x == -1 && game->map.player.position.y == -1)
		ft_error(game, 200);
	if (game->map.exit.position.x == -1 && game->map.exit.position.y == -1)
		ft_error(game, 202);
	check_border(game);
